...
/////////////////////////////
...
#endif
    th_paras *para_list = NULL;
    pthread_t *pid;
    pid = (pthread_t*)malloc(thread * sizeof(pthread_t));
    para_list = (th_paras *)malloc(thread * sizeof(th_paras));

    for(i = 0;i < thread; i++)
    {
        if(i == 0)
        {
            para_list[i].upper_value = num_count[i];
            para_list[i].lower_value = 0;
            para_list[i].id = i;
            para_list[i].UNSORTED = &final_array[0];
        }
        else
        {
            para_list[i].upper_value = num_count[i];
            para_list[i].lower_value = num_count[i-1];
            para_list[i].id = i;
            para_list[i].UNSORTED = &final_array[num_count[i-1]];   
    ...
    {
        // create the thread
        pthread_create( &pid[i], NULL, bucket_sort, (void *)( &para_list[i]) );
    }
    ...
    printf("-----step[pthread do qsort] using time :%lf-----\n", dotime);
#endif   
    free(pid);
    free(para_list);
    free(spliter);
